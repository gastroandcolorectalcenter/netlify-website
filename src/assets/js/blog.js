  ...
  const createTableOfContents = () => {
    ...
    Array.from(article.children).forEach(child => {
      if (!child.dataset.lang) {
        ...
        headings.forEach(h => englishHeadings.push(h));
        
        // Also check if child itself is a heading
        if ((child.tagName === 'H2' || child.tagName === 'H3') && !child.dataset.lang) {
          englishHeadings.push(child);
        }
      }
    });
    ...
    const nepaliHeadings = nepaliDiv ? Array.from(nepaliDiv.querySelectorAll('h2, h3')) : [];
    
    // Create English TOC
    if (englishHeadings.length >= 3) {
      ...
      englishHeadings.forEach((heading, index) => {
        ...
        // Remove # symbol from heading text if present
    ...
    // Create Nepali TOC
    if (nepaliHeadings.length >= 3 && nepaliDiv) {
      const tocContainer = document.createElement('div');
      tocContainer.className = 'table-of-contents';
      tocContainer.setAttribute('data-toc-lang', 'ne');
      tocContainer.style.display = 'none'; // Hidden by default
      tocContainer.innerHTML = '<h3>सामग्री तालिका</h3><ul class="toc-list"></ul>';
      
      const tocList = tocContainer.querySelector('.toc-list');
      
      nepaliHeadings.forEach((heading, index) => {
        if (!heading.id) {
          heading.id = `ne-heading-${index}`;
        }
